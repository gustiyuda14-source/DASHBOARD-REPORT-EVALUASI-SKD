...
function renderProfile(students, idx, tos) {
  ...
  profileCharts.push(new Chart(document.getElementById('chartProfile'), {
    ...
    options: { responsive: true, maintainAspectRatio: false, spanGaps: false, scales: { y: { min: 0, max: 500, grid: { color: 'rgba(0,0,0,.05)' } }, x: { grid: { color: 'rgba(0,0,0,.05)' } } }, plugins: { legend: { position: 'bottom', labels: { padding: 10, font: { size: 10 } } } } }
  }));
  ...
  profileCharts.push(new Chart(document.getElementById('chartRadar'), {
    ...
    options: { responsive: true, maintainAspectRatio: false, scales: { r: { min: 0, max: 230, grid: { color: 'rgba(0,0,0,.07)' }, pointLabels: { font: { size: 10 } }, ticks: { stepSize: 50, font: { size: 9 }, backdropColor: 'transparent' } } }, plugins: { legend: { position: 'bottom', labels: { padding: 10, font: { size: 10 } } } } }
  }));
}
